...
import {UserForm} from '../user-form.config';
...
export class UserEditComponent implements OnInit {
  ...
  initForm(user?: User) {
    ...
    if (this.editMode && user) {
        ...
        formData.groupList = user.groupList.toString();
        formData.channelList = user.channelList.toString();
        formData.adminGroupList = user.adminGroupList.toString();
    }
    ...
    this.groupList.map(group => {
      formControls[group.groupName] = new FormControl(formData.ofGroupAdminInRole);
    });
